...
/*
25个关键字也就是保留字，不能作为标识符。
程序声明的关键字：import,package
程序实体声明和定义的关键字：chan,func,interface,map,struct,type,var,const
5个chan,func,interface,map,struct用于定义复合数据类型  分别对应  Channel通道 Function函数 Interface接口 Map字典 Struct结构体
type用于自定义数据类型，与上面5个可以连用
var用于声明任何Go语言数据类型的变量
const用于声明常量和常量表达式
程序流程控制的关键字:go,select,break,case,continue,default,defer,else,fallthrough,for,goto,if,range,return,switch
特有的关键字：go,select主要用于并发编程
*/
